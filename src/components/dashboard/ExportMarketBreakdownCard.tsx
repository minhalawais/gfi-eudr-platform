...
import type { DashboardExportMapVM } from "@/lib/dashboard-metrics";
...
function inferRegionFromCoords([lon, lat]: [number, number]) {
  ...
  if (lat < 35 && lat > -50 && lon >= -25 && lon <= 60) return "Local";
...
export function ExportMarketBreakdownCard({ map }: { map: DashboardExportMapVM }) {
  ...
  const data = useMemo(() => {
    ...
    const desiredOrder = ["Europe", "Gulf", "Local"];
    const orderedData: { label: string; value: number }[] = [];

    desiredOrder.forEach((label) => {
      orderedData.push({ label, value: regionMap.get(label) ?? 0 });
      regionMap.delete(label);
    });

    // Add any remaining regions if present
    regionMap.forEach((value, label) => {
      orderedData.push({ label, value });
    });

    return orderedData.map((item, idx) => ({
      ...item,
  ...
  return (
    <Card className="relative flex h-full flex-col gap-2 overflow-hidden border border-border-soft bg-bg-surface p-4 shadow-glow">
      <div className="flex items-center justify-between gap-3">
        <h3 className="flex items-center gap-2 text-base font-bold leading-snug text-brand-primary">Export Market Breakdown</h3>
        <Tag tone="neutral" className="shrink-0 text-[10px] font-bold">{map.activeCountryCount} countries</Tag>
      </div>
      ...
      <div className="flex-1 flex items-center justify-between min-h-0 gap-6 mt-2">
                    ...
                    return (
                      <div className="bg-white/95 backdrop-blur-sm border border-border-soft rounded-xl px-3 py-2 shadow-xl">
                        ...
                        <p className="text-[10px] text-text-secondary">
                          {item.value.toLocaleString()} shipments ({pct}%)
                        </p>
                      </div>
                    );
                  }
        ...
        {/* Custom Legend */}
        <div className="w-28 flex flex-col gap-1.5 pl-2 select-none">
          {data.map((item, idx) => {
...
export default ExportMarketBreakdownCard;
